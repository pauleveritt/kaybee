...
class TestGenericpageAction:
    ...
    def test_construction(self, genericpage_kb_app):
    ...
    def test_identifiers_conflict(self, genericpage_kb_app, conflicting_gps):
        with pytest.raises(dectate.error.ConflictError):
            dectate.commit(genericpage_kb_app)

    def test_get_builtin_genericpage(self, genericpage_kb_app):
        ...
        dectate.commit(genericpage_kb_app)
        gp = GenericpageAction.get_genericpage(genericpage_kb_app)
        assert 'Genericpage' == gp.__name__

    def test_get_single_genericpage(self, genericpage_kb_app, valid_gp):
        # The docs project (or a third-party plugin) registers a single
        # "override"
        gp = GenericpageAction.get_genericpage(genericpage_kb_app)
        assert valid_gp == gp

    def test_get_sorted_genericpage(self, genericpage_kb_app, valid_gps):
        gp = GenericpageAction.get_genericpage(genericpage_kb_app)
